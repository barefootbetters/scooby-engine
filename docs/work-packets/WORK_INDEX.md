...
| WP | Title | Status | Deps | EC | Notes |
|---|---|---|---|---|---|
| [WP-001](WP-001-ghidra-session.md) | Ghidra session: `Scooby.exe` imports + `TGIFILE.ART` decode trace | 🚧 In Progress | WP-003 ✅, WP-007 ✅ (recommended) | [EC-001](../execution-checklists/EC-001-ghidra-session.md) | EC-001 Step 1 (PE Rich Header) ✅ done — toolchain identified across 4 titles. WP-007 baseline ✅ — Step 3 (Imports table review) is now a cross-check against [scooby-exe §Import table](../formats/scooby-exe.md#import-table-wp-007), not a re-extraction. WP-003 baseline ✅ — palette-hunt scope drops from the TGIFILE.ART decode trace; the OBJ→asset-entry 1:1 mapping cuts asset-resolution scope. **WP-008 baseline ✅** — four new explicit Ghidra targets surfaced (per [WP-001 §Background](WP-001-ghidra-session.md#background)): the `object.ini` consumer (switch on `ID=` value + name-hash lookup, xref-anchored on `Failed to load room.`), the `scrappyid=Global.Scrappy.*` script-table resolver, the inventory-anim resolver (`OBJ_INV_*` `toolbaranim=`/`usecursor=` names live outside TGIFILE.ART), and the scripted-room render branch (21 rooms with no TGIFILE.ART entry render via per-room code like `Horseshoe_Corral_Room.cpp`). Steps 2, 4, 5 (Ghidra load, file-I/O labeling, TGIFILE.ART decode trace) still pending. |
...
| [WP-004](WP-004-audio-archive-decode.md) | Audio archive index + codec identification | 📝 Drafted | — | — | Phase 1 exit criterion #3. **WP-008 baseline ✅** — `Scooby.eng` is NOT in-game dialogue (10 boot/init system-error messages only). Dialogue text store is a new unknown — most likely `Voice.dat`, embedded in `Scooby.exe`, or a not-yet-identified file. `pickupsfx=<none>` is the only audio key in 215 `object.ini` OBJ sections, weakening "voice clips referenced by name from `object.ini`" — audio cues are likely triggered by the `scrappyid` script system (WP-001 lead). See [WP-004 §Notes](WP-004-audio-archive-decode.md#notes). |
