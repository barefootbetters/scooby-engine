...
**One preventable gap.** The §Dependency check section listed only "Python 3 + `pefile`" — the executing session found Python 3.12 on PATH but resolved to the bundled `C:\Program Files\Inkscape\bin\python.exe`, which had no `pip` and no `pefile`. The user-level Python 3.12 at `C:\Users\jjensen\AppData\Local\Programs\Python\Python312\` was reachable via the `py -3` launcher. Resolution was one `py -3 -m pip install pefile` command, ~30 seconds; not load-bearing for the WP outcome. **Pre-flight follow-up for future tool-using WPs:** when a WP requires a Python package, the §Dependency check should specify the launcher (`py -3` on Windows multi-Python systems) and the install command, not just "Python 3 + `<package>`". The session prompt's §Pre-execution checks could include `py -3 -c "import <package>"` as a concrete verification step rather than `python -c ...`, which on Windows can pick up a non-user Python that lacks the package.

Otherwise the WP-007 session executed cleanly against the pre-flight contract — scope-allowlist held, determinism verified on Showdown (re-run produced byte-identical strings/imports outputs), all four binaries hashed and matched the Binary identity table, raw extractions stayed under `tools/exes/<title>/` (gitignored), and the two-commit topology was followed. The cross-title runtime-dependencies table received a load-bearing correction (Jinx does NOT statically import Bink/Smacker/QuickTime — the prior table cell read disc DLL presence as runtime usage; the imports are the load-bearing evidence) which is the kind of finding the WP was scoped to surface.
